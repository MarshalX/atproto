...
class AsyncClient(
    ...
    async def login(
        ...
        if fetch_bsky_profile:
            self.me = await self._fetch_bsky_profile(session.handle)

        return self.me

    async def _fetch_bsky_profile(self, handle: str) -> t.Optional['models.AppBskyActorDefs.ProfileViewDetailed']:
        try:
            return await self.app.bsky.actor.get_profile(models.AppBskyActorGetProfile.Params(actor=handle))
