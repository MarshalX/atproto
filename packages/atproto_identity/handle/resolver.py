...
class HandleResolver:
    def __init__(
        ...
        self._dns_resolver_async = dns.asyncresolver
    ...
    async def resolve_async(self, handle: str) -> t.Optional[str]:
        dns_resolve = await self.resolve_dns_async(handle)
        if dns_resolve:
            return dns_resolve

        http_resolve = await self.resolve_http_async(handle)
        if http_resolve:
            return http_resolve
    ...
    async def ensure_resolve_async(self, handle: str) -> str:
        did = await self.resolve_async(handle)
        if not did:
            raise DidNotFoundError(f'Unable to resolve handle: {handle}')

        return did

    async def resolve_dns_async(self, handle: str) -> t.Optional[str]:
        try:
            answers = await self._dns_resolver_async.resolve(
    ...
    async def resolve_http_async(self, handle: str) -> t.Optional[str]:
        try:
            response = await self._http_client_async.get(
                f'https://{handle}{_ATPROTO_WELL_KNOWN_PATH}', timeout=self._timeout
            )
            response.raise_for_status()
            return self._get_did_from_text_response(response.text)
        except httpx.HTTPError:
